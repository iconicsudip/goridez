...
export default function TransfersClient({ roundTrips, airportTransfers, cities }: any) {
  ...
  return (
    <>
        ...
        <div className="flex gap-2">
          {activeTab === 'ROUND_TRIP' ? (
            <button
              ...
              className="bg-green-600 hover:bg-brand-hover text-black px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest flex items-center gap-2 transition-all shadow-[0_0_15px_rgba(196,240,0,0.15)]"
            >
      ...
      <div className="flex gap-2 mb-8 border-b border-gray-300 pb-4">
        <button
          ...
          className={`flex items-center gap-2 px-6 py-3 rounded-xl text-[11px] font-bold uppercase tracking-widest transition-all ${
            activeTab === 'ROUND_TRIP'
              ? 'bg-green-600 text-white shadow-[0_0_20px_rgba(196,240,0,0.2)]'
      ...
      {activeTab === 'ROUND_TRIP' && (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          {roundTrips.map((route: any) => (
            <div key={route.id} className={`bg-gray-100 border border-gray-200 rounded-2xl p-6 group transition-all hover:border-gray-300 ${deletingId === route.id ? 'opacity-50 pointer-events-none' : ''}`}>
              <div className="flex justify-between items-start mb-4">
                <div>
                  <div className="text-[10px] text-green-700 font-mono uppercase tracking-widest mb-1">{cityMap[route.cityId] || 'Unknown City'}</div>
                  <h3 className="text-sm font-black uppercase">{route.routeTitle}</h3>
                  <div className="text-[10px] text-gray-500 font-mono mt-1">{route.distanceKm} KM Approx. • ₹{route.nightAllowance} Night Allowance</div>
                </div>
                <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => { setEditingRoundTrip(route); setIsRoundTripDrawerOpen(true); }} className="p-2 rounded-xl bg-white/5 hover:bg-green-600/10 hover:text-green-700 text-gray-500 transition-colors">
                    <Pencil size={14} />
                  </button>
              ...
              <div className="grid grid-cols-4 gap-2 text-[9px] font-mono uppercase tracking-widest text-center mt-4">
                <div className="bg-white/50 border border-gray-200 rounded-lg p-2">
                  <div className="text-gray-400 mb-1">Sedan</div>
                  <div className="text-gray-900">₹{route.sedan1D} / ₹{route.sedan2D}</div>
                </div>
                <div className="bg-white/50 border border-gray-200 rounded-lg p-2">
                  <div className="text-gray-400 mb-1">SUV</div>
                  <div className="text-gray-900">₹{route.suv1D} / ₹{route.suv2D}</div>
                </div>
                <div className="bg-white/50 border border-gray-200 rounded-lg p-2">
                  <div className="text-gray-400 mb-1">Crysta</div>
                  <div className="text-gray-900">₹{route.crysta1D} / ₹{route.crysta2D}</div>
                </div>
                <div className="bg-white/50 border border-gray-200 rounded-lg p-2">
                  <div className="text-gray-400 mb-1">Luxury</div>
                  <div className="text-gray-900">₹{route.luxury1D} / ₹{route.luxury2D}</div>
                </div>
              </div>
            </div>
          ))}
          {roundTrips.length === 0 && (
            <div className="col-span-full py-12 text-center border border-dashed border-gray-300 rounded-2xl">
              <MapPin size={24} className="mx-auto text-gray-400 mb-3" />
              <p className="text-[11px] text-gray-500 font-mono uppercase">No round trip routes configured.</p>
            </div>
          )}
        </div>
      )}
      ...
      {activeTab === 'AIRPORT' && (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          {airportTransfers.map((transfer: any) => (
            <div key={transfer.id} className={`bg-gray-100 border border-gray-200 rounded-2xl p-6 group transition-all hover:border-gray-300 ${deletingId === transfer.id ? 'opacity-50 pointer-events-none' : ''}`}>
              <div className="flex justify-between items-start mb-4">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-[10px] text-green-700 font-mono uppercase tracking-widest">{cityMap[transfer.cityId] || 'Unknown City'}</span>
                    <span className="text-[9px] bg-white/10 text-gray-600 px-2 py-0.5 rounded uppercase">{transfer.airport}</span>
                ...
                <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => { setEditingAirport(transfer); setIsAirportDrawerOpen(true); }} className="p-2 rounded-xl bg-white/5 hover:bg-green-600/10 hover:text-green-700 text-gray-500 transition-colors">
                    <Pencil size={14} />
                  </button>
          ...
          {airportTransfers.length === 0 && (
            <div className="col-span-full py-12 text-center border border-dashed border-gray-300 rounded-2xl">
              <Plane size={24} className="mx-auto text-gray-400 mb-3" />
              <p className="text-[11px] text-gray-500 font-mono uppercase">No airport transfers configured.</p>
            </div>
          )}
        </div>
      )}
