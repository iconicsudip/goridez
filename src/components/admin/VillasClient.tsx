...
function VillaDrawer({ isOpen, onClose, villa, cities, mode }: any) {
  ...
  const accent = mode === 'edit' ? 'text-yellow-400' : 'text-brand-neon';
        ...
        <div className="p-8 md:p-10">
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            <div className="space-y-2">
              <label className="text-[9px] text-white/40 font-mono uppercase tracking-widest block">Villa / Property Name</label>
            ...
            <div className="grid grid-cols-1 gap-5">
              <div className="space-y-2">
                <label className="text-[9px] text-white/40 font-mono uppercase tracking-widest block">Location / Address</label>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-5">
              <div className="space-y-2">
                <label className="text-[9px] text-white/40 font-mono uppercase tracking-widest block">Starting Price / Night (₹)</label>
                <input name="startingPrice" required type="number" defaultValue={villa?.startingPrice} placeholder="24000"
                  className="w-full bg-[#111111] border border-white/5 rounded-xl px-4 py-3 text-xs text-white outline-none font-mono focus:border-white/20" />
              </div>
              <div className="space-y-2">
                <label className="text-[9px] text-white/40 font-mono uppercase tracking-widest block">Max Occupancy (Guests)</label>
            ...
            {/* Amenities Tag Input */}
            <div className="space-y-2">
              <label className="text-[9px] text-white/40 font-mono uppercase tracking-widest block">Amenities</label>
              <div className="relative">
                <div className="min-h-[52px] bg-[#111111] border border-white/5 rounded-xl px-3 py-2 flex flex-wrap gap-2 focus-within:border-brand-neon/40 cursor-text"
                  onClick={() => document.getElementById('amenity-input')?.focus()}>
                  {amenities.map(a => (
                    <span key={a} className="flex items-center gap-1.5 bg-brand-neon/10 border border-brand-neon/30 text-brand-neon text-[9px] font-bold uppercase px-3 py-1 rounded-lg">
                      <Tag size={9} />{a}
                      <button type="button" onClick={() => setAmenities(prev => prev.filter(x => x !== a))} className="text-brand-neon/50 hover:text-red-400 ml-1"><X size={9} /></button>
                    </span>
                  ))}
                  <input id="amenity-input" value={amenityInput}
                    ...
                    className="flex-1 min-w-[160px] bg-transparent outline-none text-xs text-white font-mono py-1 placeholder:text-white/20" />
                </div>
    ...
    <>
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8">
        <div>
          ...
          <p className="text-[10px] text-white/40 font-mono tracking-widest uppercase">Manage luxury villa listings, amenities, and pricing.</p>
        </div>
        <div className="flex gap-3 w-full md:w-auto">
          ...
          <button onClick={() => setShowAdd(true)}
            className="bg-brand-neon hover:bg-brand-hover text-black px-5 py-3 rounded-xl text-[11px] font-black uppercase tracking-widest flex items-center gap-2 transition-all whitespace-nowrap">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-5">
        {filtered.map(villa => {
          ...
          return (
            <div key={villa.id} className={`bg-[#111111] border border-white/5 hover:border-white/10 rounded-2xl overflow-hidden group transition-all ${isDeleting ? 'opacity-40 pointer-events-none' : ''}`}>
              <div className="relative w-full h-40 bg-black overflow-hidden">
                ...
                <div className="absolute bottom-3 left-3">
                  <div className="text-brand-neon font-black text-lg">₹{villa.startingPrice.toLocaleString()}<span className="text-[10px] font-normal text-white/60">/night</span></div>
                </div>
              </div>
              <div className="p-5">
                <h3 className="font-black text-sm uppercase tracking-tight mb-1">{villa.name}</h3>
                <div className="flex items-center gap-3 text-[9px] font-mono text-white/40 mb-3">
                ...
                <div className="flex flex-wrap gap-1.5 mb-4">
                  {amenities.slice(0, 3).map((a: string) => (
                    <span key={a} className="text-[8px] font-mono text-white/50 bg-white/5 px-2 py-1 rounded-lg border border-white/5">{a}</span>
                  ))}
                  {amenities.length > 3 && <span className="text-[8px] font-mono text-white/30 px-2 py-1">+{amenities.length - 3}</span>}
                </div>
                <div className="flex justify-end gap-2 pt-3 border-t border-white/5">
                  <button onClick={() => setEditingVilla(villa)} className="w-9 h-9 rounded-xl bg-white/5 hover:bg-brand-neon/10 hover:text-brand-neon text-white/40 flex items-center justify-center transition-colors border border-white/5"><Pencil size={14} /></button>
