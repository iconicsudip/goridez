  ...
  return (
    <div className="container">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-10 border-b border-gray-200 pb-6">
        <div>
          <h1 className="text-4xl font-black uppercase tracking-tight mb-2 text-gray-900">
      ...
      <div className="bg-gray-100 border border-gray-200 rounded-3xl p-8 mb-8">
        <div className="flex justify-between items-end border-b border-gray-300 pb-4 mb-4">
          <div className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Story Title</div>
          <div className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">Parameters</div>
        </div>

        <div className="space-y-4">
          {blogs.length === 0 ? (
            <div className="text-[11px] text-gray-400 italic text-center py-6 font-mono">
              No stories found.
            </div>
          ) : (
            blogs.map((blog) => (
              <div key={blog.id} className="flex items-center justify-between py-4 border-b border-gray-200 last:border-0 group">
                <div>
                  <h3 className="font-bold text-sm mb-1 text-gray-900">{blog.title}</h3>
                  <div className="text-[10px] text-gray-500 font-mono">Slug: /blogs/{blog.slug}</div>
                </div>

                <div className="flex items-center gap-6">
                  <div className="flex items-center gap-3">
                    <span className="text-[9px] font-black uppercase tracking-widest px-3 py-1 rounded border border-brand-gold/30 text-green-700">
                      {blog.category}
                    </span>
                    <button
                      onClick={() => togglePublish(blog.id, blog.isDraft)}
                      className={`text-[9px] font-black uppercase tracking-widest transition-colors ${blog.isDraft ? 'text-gray-400 hover:text-gray-600' : 'text-[#00FF66] hover:text-[#00CC55]'}`}
                    >
                  ...
                  <div className="flex items-center gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                    <button onClick={() => handleEdit(blog)} className="text-gray-500 hover:text-gray-900 transition-colors">
                      <Edit2 size={16} />
                    </button>
                    <button onClick={() => handleDelete(blog.id)} className="text-gray-500 hover:text-red-500 transition-colors">
                      <Trash2 size={16} />
                    </button>
                  </div>
                </div>
      ...
      {isDrawerOpen && (
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          {/* Drawer Container */}
          <div className="relative w-full max-w-2xl bg-gray-100 border-l border-gray-300 h-full p-8 overflow-y-auto shadow-2xl flex flex-col justify-between z-10 animate-slide-in">
            <div className="flex flex-col h-full justify-between">
              <div>
                <div className="flex justify-between items-center mb-8 border-b border-gray-200 pb-4">
                  <div>
                    <span className="text-[9px] text-green-700 font-black tracking-widest uppercase mb-1 block">Content Management</span>
                ...
                <form onSubmit={handleSubmit} className="space-y-6">
                    ...
                    <div className="bg-gray-50 rounded-xl border border-gray-300 overflow-hidden focus-within:border-green-600 transition-colors">
                      ...
                      <div className="bg-gray-100 border-b border-gray-300 p-2 flex flex-wrap gap-1">
                        {[
                          ...
                          { label: 'Link', open: '<a href="#" class="text-green-700 underline">', close: '</a>' },
                      ...
                      <textarea
                        ...
                        className="w-full bg-transparent p-4 min-h-[300px] font-mono text-xs text-gray-900 outline-none border-0 resize-y"
                      />
                    </div>
                  </div>
                  ...
                  <div className="flex gap-4 pt-4 border-t border-gray-200 mt-8">
                    <button
                      type="submit"
                      disabled={loading}
                      className="flex-1 bg-green-600 hover:bg-brand-gold-hover text-white py-4 rounded-xl text-[11px] font-black tracking-widest uppercase transition-all shadow-md disabled:opacity-50"
                    >
