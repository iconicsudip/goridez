...
import { Sparkles, Save, Wand2, ExternalLink, CheckCircle2, AlertCircle } from 'lucide-react';
import { updateLlmsTxt, generateLlmsTxtDraftAction } from '@/app/admin/actions';
...
export default function LlmsTxtManager({ isCustomized, initialLlmsTxt }: Props) {
  const router = useRouter();
  const [text, setText] = useState(initialLlmsTxt);
  ...
  const save = async () => {
    setSaving(true);
    setMsg(null);
    const res = await updateLlmsTxt(text);
    setSaving(false);
    if (res.success) {
      setMsg({ type: 'success', text: 'llms.txt saved — live at /llms.txt' });
      router.refresh();
    } else {
  ...
  const generate = async () => {
    ...
    if (res.success && res.draft) {
      setText(res.draft);
      setMsg({ type: 'success', text: 'Fresh draft generated from live site content below — review and click Save to publish it.' });
    } else {
      setMsg({ type: 'error', text: res.error || 'Failed to generate draft' });
    }
  };
    ...
    <div className="max-w-6xl mx-auto py-6 font-body">
      ...
      <div className="mb-8 border-b border-gray-200 pb-6">
        <div className="flex items-center gap-2 text-green-700 font-mono text-xs font-bold uppercase tracking-wider mb-1">
          <Sparkles size={16} /> AI SEO
        </div>
        <h1 className="text-3xl font-black text-gray-900 uppercase font-serif tracking-tight">LLMs.txt</h1>
        <p className="text-gray-500 text-sm mt-1">A structured summary of the site for AI assistants to read.</p>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-3xl p-8 shadow-sm">
        <div className="flex items-start justify-between gap-4 mb-2">
          <div>
            ...
            <p className="text-gray-500 text-xs mt-1 max-w-xl leading-relaxed">
              Plain-language markdown links describing the site for AI assistants &amp; LLM crawlers (the <a href="https://llmstxt.org" target="_blank" rel="noreferrer" className="underline">llms.txt standard</a>) — services, vehicles, cities, tours and articles.
            </p>
          </div>
        ...
        <div className="flex justify-end gap-3 mt-4">
          ...
          >
            <Wand2 size={14} /> {generating ? 'Generating…' : 'Generate Draft from Site Content'}
          </button>
          <button
            type="button"
            onClick={save}
          ...
          >
            <Save size={14} /> {saving ? 'Saving…' : 'Save llms.txt'}
          </button>
        </div>
      </div>
    </div>
